...
public class SimpleArm : Agent
{
    ...
    public override void OnActionReceived(ActionBuffers actions)
    {
        ...
        controlSignals[0] = new Vector3(0, 0, actions.ContinuousActions[0] * Speed);
        controlSignals[1] = new Vector3(0, actions.ContinuousActions[1] * Speed, 0);
        controlSignals[2] = new Vector3(0, actions.ContinuousActions[2] * Speed, 0);
        controlSignals[3] = new Vector3(0, actions.ContinuousActions[3] * Speed, 0);

        /*
        rNode[0].velocity = Vector3.zero;
        rNode[0].angularVelocity = Vector3.zero;

        float Current = nodes[0].localEulerAngles.z;
        if (Current + controlSignals[0].z > 350)
        {
            nodes[0].localEulerAngles = new Vector3(0, 0, 350);
        }
        else if (Current + controlSignals[0].z < 10) 
        {
            nodes[0].localEulerAngles = new Vector3(0, 0, 10);
        }
        else
        {
        ...
        */
        ...
        Vector3 newangle = nodes[0].localEulerAngles + (controlSignals[0]);
        ...
        for (int i=1; i<4; i++)
        {
            ...
            newangle= nodes[i].localEulerAngles + (controlSignals[i]);
